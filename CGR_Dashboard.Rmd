...
```{r, include=FALSE}
#ACOTAR LAS TABLAS A SOLO LAS COLUMNAS QUE EFECTIVAMENTE SE USAN
```
...
renderPlotly({
        ...
        PDE_categories_historical <- get(load('out/PDE_categories_df_historical.Rdata'))
        
        if (input$year_selection == "Todos los años") {
                
                pde_status_df <- as.data.frame(table(PDE_categories_historical$PDE_decision_categories))
                colnames(pde_status_df) <- c("pde_status", "n")

        } else {
                
                PDE_categories_historical <- subset(PDE_categories_historical, year== input$year_selection)
